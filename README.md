...
npm run build      # astro check + білд + Pagefind + service-worker
...
Білд також генерує `dist/sw.js` (service-worker) і `public/manifest.webmanifest` —
сайт встановлюваний як PWA. Реєстрація SW активна лише у проді
(`import.meta.env.PROD`-гейт у `Base.astro`); у dev-режимі він не реєструється,
щоб не плутати HMR. Деталі — у [`docs/architecture.md`](docs/architecture.md)
(розділ «PWA»).
