...
Останнє оновлення: травень 2026 року, після PR з roadmap-фічами (#2, #3, #4, #5, #7, #9, #11, #12, #17, #19, #22).
...
```
content/uk/                     ← i18n-ready: окремо від коду; те, що змінюється на форку
...
1. Клонуй репо.
2. Заміни `content/uk/eras.json` на власний параметр (наприклад, «рік народження → покоління»).
3. Заміни `content/uk/facts/` на свій тематичний контент (з frontmatter за схемою).
...
| 2   | Шер-кнопки + копіювання посилання     | ✓     | DONE (`ShareBar.astro` на `/[year]` і `/share/...`) |
| 3   | Лічильник «База: N фактів» у хедері   | ✓     | DONE (`Header.astro`)                               |
| 4   | Анімовані тізери на лендингу          | ✓     | DONE (3 тізери з різних предметів)                  |
| 5   | Сторінка `/share/[year]` (PNG-постер) | ✓     | DONE (canvas-генерація PNG)                         |
...
| 7   | Прив'язка до тем шкільної програми | ✓          | DONE (`src/lib/topics.ts`, `/temy.html`)           |
| 8   | Розділ «Підручники» з обкладинками | ⊘ (можемо) | Pending                                            |
| 9   | `/timeline` — хронологія           | ✓          | DONE (`/timeline.html` згрупована за роками й ерами) |
...
| #   | Пункт                                | Згода       | Статус                      |
| --- | ------------------------------------ | ----------- | --------------------------- |
| 11  | Квіз «вгадай рік відкриття»          | ✓           | DONE (`/kviz.html`, 5-питальні раунди)        |
| 12  | «Тоді vs зараз» (порівняльна картка) | ✓ обов'яз.  | DONE (frontmatter `comparison` + UI у картці) |
...
| 17  | Подяки на about-сторінці      | ⊘ (можна)     | DONE (`/about.html#podyaky`) |
...
| 19  | Plausible / simpleanalytics           | ✓     | DONE (env-driven у `Base.astro`: `PUBLIC_PLAUSIBLE_DOMAIN` / `PUBLIC_SIMPLEANALYTICS`)                                              |
| 20  | Decap / Sveltia CMS поверх `content/` | ✓     | DONE (`public/admin/config.yml` з полями comparison/topic/lang)                                                                     |
...
| 22  | i18n-ready (`content/uk/`)            | ✓     | DONE (контент перенесено в `content/uk/`, схеми оновлено)                                                                           |
...
### Сесія 8 — Roadmap-фічі (батч 1: інфра + інтерактив)

Працюємо на гілці `devin/1778150548-roadmap-features`. Зроблено:

- **#22 i18n-готовність:** контент переїхав з `content/` у `content/uk/` (eras.json + 75 фактів × 5 предметів). Оновлено `src/content.config.ts`, `scripts/content-utils.ts`, `src/lib/eras.ts`. Готовий ґрунт під `content/en/` без змін у коді сторінок.
- **#3 лічильник у хедері:** `Header.astro` рахує `getCollection("facts")` й виводить «База: N фактів» у мобільній і десктопній версіях.
- **#19 аналітика:** `Base.astro` під'єднує Plausible / SimpleAnalytics через env (`PUBLIC_PLAUSIBLE_DOMAIN`, `PUBLIC_SIMPLEANALYTICS`). За замовчанням нічого не вантажиться.
- **#12 «Тоді vs зараз»:** у схемі факту з'явилось опційне `comparison: { then, now }`; `FactCard.astro` рендерить дві колонки. Демо-приклади додані до `pluto-not-planet.md`, `crispr-cas9.md`, `higgs-boson.md`.
- **#7 Теми шкільної програми:** `src/lib/topics.ts` тримає 20 курікулумних тем, мапінг по предметах. У схемі факту з'явилось опційне `topic`. Сторінка `/temy.html` групує факти по темах із прив'язкою до предмета й кількістю фактів.
- **#2 Шер-кнопки:** `ShareBar.astro` (Telegram, Threads, X, Facebook, copy-link) на сторінці року й на постері.
- **#5 PNG-постер `/share/[year]`:** окрема сторінка генерує PNG через canvas (DPR-aware, fallback texture).
- **#9 `/timeline.html`:** хронологія всіх фактів, згрупована по ерах і роках.
- **#11 `/kviz.html`:** «вгадай рік відкриття» — раунди по 5 питань, дистрактори генеруються детерміністично.
- **#4 Анімовані тізери:** на лендингу 3 свіжих факти з різних предметів зі staggered fade-in (з `prefers-reduced-motion: no-preference`).
- **#17 Подяки:** в `about.astro` додано блок з джерелами (Nature, NASA, ESA, CERN, LIGO, IAU, IUPAC, BIPM, архіви України), інструментами (Astro/TS/Tailwind/шрифти/хостинг) і подякою вчителям.
- **#20 CMS:** у `public/admin/config.yml` додано поля `comparison`, `topic`, `lang` — Decap-редактор бачить нові поля.
...
| Демо-`comparison` для решти 72 фактів     | Зроблено для 3 sample-фактів; решта — інкрементально по батчах.      |
