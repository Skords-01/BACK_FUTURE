...
- [#36](https://github.com/Skords-01/BACK_FUTURE/pull/36) — фаза 2.2: опц. поле `region: "world" | "ukraine"` у `content.config.ts`. На FactCard для `ukraine` малюється прапор ї «УКРАЇНСЬКИЙ ВНЕСОК» (`title` для hover, `aria-label`); `world` — нічого не рендериться. Гайд по content `region` в `docs/content-guidelines.md`.
