...
Останнє оновлення: травень 2026 року, після UX-батчу PR #20–24 (OG, share, header counter + breadcrumb, decade chips, a11y).
...
| ------------ | ------------------------------ | --------------------------------------------------------- |
| Astro        | 6.x                              | Static-first, 0 KB JS за замовчанням, content collections |
| TypeScript   | 5.7.2 (strict)                   | Безпека типів, краще DX                                   |
| Tailwind CSS | 4.x (через `@import "tailwindcss"`) | Дизайн через утилітарні класи, токени в одному файлі      |
| Zod          | 4.x                              | Валідація frontmatter на білд-тайм                        |
| Node         | 22.13.0 (фіксується через `.nvmrc`) | Підтримуваний Astro 6                                  |
...
| #   | Пункт                                 | Згода | Статус                                                      |
| --- | ------------------------------------- | ----- | ----------------------------------------------------------- |
| 1   | Динамічна OG-картка по року           | ✓     | DONE — PR #20 (facts count + субʼєктні pills + N оновлень) |
| 2   | Шер-кнопки + копіювання посилання     | ✓     | DONE — PR #21 (Telegram/X/Facebook + clipboard + Web Share) |
| 3   | Лічильник «База: N фактів» у хедері   | ✓     | DONE — PR #22 (badge + breadcrumb на `/[year]`)            |
...
### Сесія 8 (PR #20–24) — UX-батч

Користувач попросив обрати 5 PR-ів «на свій смак» і виконати без додаткових питань. Обрано п'ять незалежних, не-контентних UX-фіч:

- **PR #20 — Динамічна OG-картка** (`src/pages/og/[year].svg.ts`)
  - Раніше: статичний шаблон з роком і назвою.
  - Зараз: фетчимо `getCollection("facts")`, рахуємо total/subjects, виводимо ряд subject-pills із емоджі та лічильником, плюс правильна українська плюралізація для «N оновлень».
  - Pills рендеряться з overflow-захистом (clip after 1080px).
- **PR #21 — Шер-кнопки** (`src/components/ShareBlock.astro` + інтеграція в `[year].astro`)
  - 3 соцмережі (Telegram/X/Facebook) + Copy + Web Share API.
  - Без зовнішніх трекерів. Web Share прихований, якщо не підтримується.
  - Статуси через `role="status" aria-live="polite"` для скрін-рідерів.
- **PR #22 — Header counter + Breadcrumb** (`src/components/Header.astro`, `src/pages/[year].astro`)
  - Лічильник «N фактів» у хедері, прихований на мобільному.
  - Breadcrumb: Головна › Випуск YYYY на `[year]`, з `aria-current="page"`.
- **PR #23 — Decade chips + кращий empty state** (`src/pages/index.astro`, `src/pages/[year].astro`)
  - Швидкий вибір року на головній (1995, 2000, …, 2026 з кроком 5).
  - На рік без фактів: великий заголовок, ряд сусідніх років (-5/-1/+1/+5), CTA з prefilled GitHub issue.
- **PR #24 — A11y & polish** (`src/layouts/Base.astro`, `src/components/Header.astro`, `src/styles/global.css`)
  - Skip-to-content link, `<main tabindex="-1">`.
  - Глобальний `:focus-visible` ring (accent.500, 2px).
  - `prefers-reduced-motion` — вимикає всі transitions/animations.
  - `aria-current="page"` на активному пункті навігації.
  - Print stylesheet — ховає хедер/футер, розгортає `<details>`, дописує URL для зовнішніх посилань.

**Технічні нотатки сесії:**

- Кожен PR незалежний від інших, гілка від `main`. Послідовний merge в довільному порядку.
- Кожен PR пройшов `npm run build` (Astro check + 40 pages) і `npm run lint` (ESLint + Prettier + markdownlint).
- Astro 6 не дозволяє shorthand-фрагмент `<>` з атрибутами в `.map()` всередині JSX — наштовхнулись у PR #23, обхід: винести `.filter().map()` у фронтматер як змінну `neighborYears`.
- `git_create_pr` tool періодично повертав «Internal error» — fallback: GitHub REST API через curl з `Git_PAT`.
