...
5. Source URLs are checked in the `Link check` workflow: CI runs `npm run dump:source-urls` and validates `dist/.fact-source-urls.md` with Lychee as a warning-only external check.
