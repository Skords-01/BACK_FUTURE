...
| 2.6 | CI-перевірка: `sources[].url` віддає 200 (link-check уже є — розширити) | S      | —        | Додати в існуючий `link-check.yml` крок із пошуку битих посилань у фактах. **In progress** (цей PR — `dump-source-urls.ts` + крок у workflow). |
...
5. **#2.7** — окремий markdownlint для `content/facts/` (S). Стрічкові правила якості для контенту.
