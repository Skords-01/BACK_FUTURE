...
Останнє оновлення: травень 2026 (після фази 2 і початку фази 3 — PR-и #35, #36, #37, #38, #39).
...
| Тех-борг      | Налаштувати pre-commit hook для prettier + markdownlint (опційно). Renovate / посилення Dependabot.            |
...
3. **#1.3** — OG як PNG (S). Закриває останній pending пункт із топ-5 «вау-фішок».
4. **#4.2** — фільтри на сторінці року (M). Тепер є чим фільтрувати: `region` + `impact` уже у схемі.
5. **#2.5** — розширення `SUBJECTS` (tech, medicine, economy, culture, sport, ecology) (M). Список + emoji + label у `site.ts`; контент додається окремо.

Залежності: #4.2 спирається на 2.2 і 2.4 (вже в main / в PR-чергу). Решту чотирьох можна паралелити.
